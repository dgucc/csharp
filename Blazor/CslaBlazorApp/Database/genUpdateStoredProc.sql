...
BEGIN
	...
	-- generate the drop
	set @SQL_DROP = @SQL_DROP + 'IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[' + @spSchema + '].[usp_' + @spTable + '_update]'') AND type in (N''P'', N''PC''))' + char(13)
	set @SQL_DROP = @SQL_DROP + 'DROP PROCEDURE [' + @spSchema + '].[usp_' + @spTable + '_update]'
	
	set @SQL = @SQL + 'CREATE PROC [' + @spSchema + '].[usp_' + @spTable + '_update]' + char(13)
	set @SQL = @SQL + '(' + char(13)
	...
	set @SQL = @SQL + 'BEGIN TRY' + char(13) + char(10) 
	...
	set @SQL = @SQL + 'WHERE ' + @PK_COLUMN + ' = @' + @PK_COLUMN + char(13) + char(10)
	
	set @SQL = @SQL + 'SELECT @' + @PK_COLUMN + char(13) + char(10)
	...
	set @SQL = @SQL + '	declare @ErrorState INT;' + char(13) + char(10)
	set @SQL = @SQL + '	SELECT @ErrorMessage = ERROR_MESSAGE(), @ErrorSeverity = ERROR_SEVERITY(), @ErrorState = ERROR_STATE();' + char(13) + char(10)
	set @SQL = @SQL + '	RAISERROR (@ErrorMessage, @ErrorSeverity, @ErrorState);' + char(13) + char(10)
	set @SQL = @SQL + '	ROLLBACK TRANSACTION' + char(13) + char(10)  
	
	set @SQL = @SQL + 'END CATCH;' + char(13) + char(10)
	
	RETURN @SQL_DROP + + char(13) + char(10) + 'GO' + char(13) + char(10) + @SQL
